...
from accounts.constants import ONLINE, OFFLINE
...
class SetStatusHandler(tornado.websocket.WebSocketHandler):
    """
    Websocket connection handler to set user status to online/offline
    """
    def open(self, user_id):
        """
        Set user profile status to online, then user connected
    ...
    def on_close(self):
        """
        Set user profile status to offline, then connection to websocket closed
        """
        try:
            profile = UserProfile.objects.get(id=self.profile_id)
        except (UserProfile.DoesNotExist):
            return
...
if __name__ == "__main__":
    ...
    app = tornado.web.Application(handlers=[
        ...
        (r"/status/(?P<user_id>\d+)/", SetStatusHandler),
    ])

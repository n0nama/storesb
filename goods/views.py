...
def good_view(request, good_pk):
    ...
    if len(random_sellers) > 0:
        ...
        # If we has just one seller with same tags - show he only
        if len(random_sellers) > 1:
            rand_seller_num = 1
        else:
            rand_seller_num = 0
        random_goods_slider = Good.objects.filter(user__profile=random_sellers[rand_seller_num]).order_by('?')[:2]
    ...
    if request.method == 'POST':
        ...
        if good_form.is_valid() and properties_form.is_valid() and image_upload_form.is_valid():
            ...
            for tag in tags:
                ...
                # Setup filtered request for redirect
                tag_req = tag_req + 'tag_id[]=' + str(tag.id) + '&'
            ...
            return redirect(reverse('goods:search') +'?filter=1&' + tag_req)
        else:
            messages.error(request, _("Error adding goods"))
    else:
        image_upload_form = CustomImageUpload()
        good_form = GoodForm(None, initial={'deal': AUCTION})  # Set initial type to auction because it's first slider

...
def simple_search(words, tags=None):
    ...
    """
    if tags:
        goods = Good.active.filter(Q(tags__in=tags) | Q(title__icontains=words) |
                                   Q(properties__description__icontains=words)).distinct().order_by('-created')
    else:
        goods = Good.active.filter(Q(title__icontains=words) |
                                   Q(properties__description__icontains=words)).distinct().order_by('-created')
    return goods
...
def filtered_search(request_method):
    ...
    """
    all_goods = Good.active.all().order_by('-created')
